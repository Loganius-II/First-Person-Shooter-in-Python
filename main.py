...
tip = Text(
    text='Controls:\nW,A,S,D: Walking\nSpace: Jump\nr: Reload\nLeft Click: Shoot\nRight Click: Aim\nShift: Sprint\nk + Left Click: Exit\nk+k: Undo Exit',
    position=(-0.78, 0.4),  # Adjust position to place text where you want
...
point_lbl = Text(
    ...
    position=(0.2, -0.47),  # Adjust position to place text where you want
    origin=(0, 0),  # Center the text
    scale=0.5,
    color=color.white    
)
...
def input(key):
    ...
    if key == "left mouse down":
        if ammo != 0 and reloading == False:
            playsound("Assets/Sounds/glock_sound.mp3", block=False)
            try:
                if mouse.hovered_entity.block == "foo":
